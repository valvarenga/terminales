...
<form method="GET" class="content-card p-3 mb-4 row g-3 align-items-end"><div class="col-md-3"><label for="entity" class="form-label">Sección</label><select id="entity" name="entity" class="form-select"><option value="">Todas</option>@foreach($entities as $value => $label)<option value="{{ $value }}" @selected(request('entity') === $value)>{{ $label }}</option>@endforeach</select></div><div class="col-md-3"><label for="q" class="form-label">Responsable o registro</label><input id="q" name="q" class="form-control" value="{{ request('q') }}" maxlength="100"></div><div class="col-md-2"><label for="from" class="form-label">Desde</label><input type="date" id="from" name="from" class="form-control" value="{{ request('from') }}"></div><div class="col-md-2"><label for="to" class="form-label">Hasta</label><input type="date" id="to" name="to" class="form-control" value="{{ request('to') }}"></div><div class="col-md-2 d-flex gap-2"><button class="btn btn-primary">Filtrar</button><a href="{{ route('admin.history') }}" class="btn btn-outline-secondary">Limpiar</a></div></form>
